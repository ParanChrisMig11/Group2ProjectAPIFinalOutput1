...
while True:
    ...
    if json_status == 0:
        ...
        if time_data.get("status") == "OK": #Check if the time data status indicates a successful response
            #Display the local time and time zone information for the destination
            print(f"Local Time at {dest}: {time_data['formatted']}")
            print(f"Time Zone: {time_data['zoneName']} (UTC{time_data['gmtOffset'] // 3600:+})")
        else: #Notify the user if time zone data is unavailable
        ...
        #Check if the current weather data is available in the response
        if "current" in weather_data:
            #Extract current weather and location information
            ...
            #Construct the forecast API URL for additional data (e.g., sunrise and sunset times)
            forecast_url = f"{forecast_api}?key={weather_key}&q={urllib.parse.quote(dest)}&days=1&aqi=no&alerts=no"
            forecast_data = requests.get(forecast_url).json() #Fetch and parse the forecast data
        ...
        else: #End of weather information section
